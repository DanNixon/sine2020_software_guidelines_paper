...
It is therefore benefical to develop code that is easy to maintain, extend (new functionality) and does not have duplicated effort.
...
Specific rules depend greatly on the language in question and as a result should be chosen based on the language that the software is going to be written in. If multiple languages are going to be used it is possible to have rules for each language, see Mantid as an example. 
...
The crucial functionality of issue tracking is to provide a description of the task and to assign it to a developer.
Additional features may include boards such as Scrum/Kanban boards (e.g.\/ JIRA (TODO provide reference)), task dependency and task hierarchy. (TODO: white board with stickers / cards)
...
Guidelines can allow any software project to fully utalise the benefits of SCM.
...
For example, Travis CI \cite{Travis_CI} offers a free service that will compile builds and run tests over a fixed time period (available for Linux, Mac OS and Windows).
...
For example, it is possible to increase/remove restrictions for build times, store data locally and complete control over the setup of the build environment.
