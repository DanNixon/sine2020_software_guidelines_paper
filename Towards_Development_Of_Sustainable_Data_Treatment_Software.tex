...
However, this should not be done at the expense of creating duplicated code.
...
C++ projects can also benefit from use of a dependency manager such as Conan \cite{} (TODO:add ref).
...
Any implementation of a standardised data format needs to either follow the standard or publish any divergence. 
...
The format has existed for roughly two decades it's widespread use as the standard data format has been a relatively recent effort between facilities and software projects.
...
It is useful to be able to extract the exact conditions of the software (i.e. the version number of the software, versions of plugins/dependencies).
...
These will meet fairly irregularly (a few times a year), but would involve the senior team memebers of the project and key steakholders (including the sponsor).
...
A large framework used by multiple different scientific techniques at different facilities (such as the Mantid project \cite{} (TODO: addcitation)) requires an involved release process to ensure the software is suitable for use by all intended audiences.
This section will assume that a significant proportion of users are not involved in the development process.
