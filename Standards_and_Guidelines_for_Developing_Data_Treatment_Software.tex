...
\begin{abstract}
TODO
For discussion: To what extend should EU SINE2020 be mentioned in the abstract?
\end{abstract}
...
A standard tends to be more specific and carry more authority and may naturally emerge from a community following guidelines in a specific context. The Oxford dictionaries list multiple definitions of standard, including "A level of quality or attainment", for example “their restaurant offers a high standard of service".
We will use these definition throughout the remainder of this paper.
...
Hence, when it is appropriate the idea of the standard/guideline will be abstracted so that a new tool can be chosen in the future.
...
Code should be of a high standard, the reason for this is two fold; good quality code that conforms to accepted standards is
more likely to be well formatted, readable and contain fewer bugs thus code that is easier to maintain in the future.
...
Ideally both levels of tracking should be available to view freely by both users and developers.
...
Larger or more critical projects may opt to have multiple review stages, in which one or more testers (that has yet to be involved in the development or testing of this proposed change) will perform the same duties as the previous testers.
Having multiple reviewers for the same piece of work may seem inefficient but should increase the chance of potential issues being found before they are released to users.
...
The ability to reproduce results of data treatment is important for users; where data treatment of raw data commonly requires the application of many corrections, transformations etc.
...
It is advisable to schedule maintenance tasks that can potentially affect many aspects of the code to immediately after releases to give the team the maximum time to iron out any unforeseen edge cases before the next code freeze.
...
For each software release it is recommended to generate a Digital Object Identifier (DOI).
...
If the deadline and time required are similar then it is better to choose a developer with existing relevant skills.
If there is sufficient time between the deadline and the estimated time to complete the task then this provides an opportunity to select a less experienced developer and an opportunity for training.
...
To ensure users approach the new version with realistic expectations it is essential to provide sufficiently detailed release notes with every version of your software.
...
As the technology becomes more popular it is worth looking into cross platform container based means of distribution (for example, Docker (TODO: link) and Singularity (TODO: link)).
This allows the shipping of software in an already configured and working state.
...
Good documentation will reduce the time spend by developers and others to explain how to use the software.
...
For example, for intricate GUIs, video tutorials may be beneficial as the user sees a complete workflow and there is less chance of steps being undocumented.
...
A key activity for any piece of software is how users are supported.
...
Further, standards for software/IT service management exist, for example FitSM (add ref), which provides more detailed information for how to manage software beyond what is presented in this section.

The documentation discussed so far has focused on the users.
It is also valuable to have developer documentation.
This documentation should contain information on the standards used in the code and how to set up a development environment, and could contain appropriate tutorials and relevant reference material.
...
There is an initial cost in deciding what those should be, and chosen based on the requirements of the project.
...
Ultimately the main benefit of software that follows standards and guidelines is that it help development to continue even if the developers change.
This is because these ensure a uniform code quality, better developer team setup and user management, which allows for a more stable and maintainable piece of software and users experiences of it.
This will increase the likelihood of the software user base growing and flourishing.
