...
Due to large volumes of data and complex analysis techniques, such a software is often expensive to develop and maintain.
...
The reason for this is at least two-fold: first, the demography of the users is changing from scientists building a career in neutron scattering or muon spectroscopy to scientists that consider neutron scattering and muon spectroscopy only as tools among a broader set of tools; they are thus often not knowledgeable about the details behind the data treatment.
Moreover, the users may not have the technical skills in how to analyse their data with programs like {\sc Matlab}, {\sc Python}, {\sc IDL}, etc. 
...
European Spallation Source (ESS) are estimated to have data rates beyond ...(\todo{TODO: Tobias paper}) and at modern photon sources the situation is even more severe.
...
The consequence of these aspects is that users will inherently depend on software developed by other developers, typically at the hosting facilities or in the user community.
This adds significant requirements in terms of quality, trustability, reliability, reproducibility and usability compared to when a user develops software for his own use.
...
In the worst case this may lead to people believing that the code is incorrect, when it is correct, or other way around.
The other potential pitfall with this isolated approach of software development is that bugs are less likely to be found, due to less users and less developers.
...
A good set of guidelines will help the software be sustainable; that is, reliable, inter-operable, extensible, intuitive, maintainable.
That way it will be easier to continue growing the code base to include new scientific analysis and techniques and add new functionality.
...
For instance, {\sc Python} is today the \textit{de facto} standard scripting language in scientific computing even though the first scientific python conference to the best of our knowledge was held as late as in 2002.
Abstracting guidelines in this manner allows to remain relevant as technology evolves.
