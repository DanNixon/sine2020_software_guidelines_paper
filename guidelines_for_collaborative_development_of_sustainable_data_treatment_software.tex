...
An increasingly common method of delivering software to users is via cloud infrastructure.
In this model the software is designed to run on dedicated compute infrastructure with an interface layer providing interaction with the user.
This layer may be a desktop based GUI, command line or web interface.
The advantage of this delivery method is that the environment is which the software runs is more closely controlled and specified by the developer, reducing the impact of many cross platform development issues.
The software may also reside closer to the data, as data volume grows this is important in removing data transfer and storage bottlenecks involved in traditional software.
A disadvantage of this method is that it add dependency on the infrastructure hosting the software.
