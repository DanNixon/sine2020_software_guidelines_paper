...
and
\author[B]{\inits{T.}\fnms{Thomas H.\/} \snm{Rod}\ead[label=e4]{Thomas.Rod@esss.se}}
...
\address[B]{\orgname{European Spallation Source ERIC}, \cny{Denmark}\printead[presep={\\}]{e4}}
\address[C]{\orgname{Paul Scherrer Institute}, \cny{Switzerland}\printead[presep={\\}]{e5}}
\address[D]{\orgname{Forschungszentrum J\"{u}lich}, \cny{Germany}\printead[presep={\\}]{e6}}
...
Therefore, it is imperative to ensure that the software will continue to be developed in the future, even if the current work force leaves the project.
...
This paper presents a set of guidelines with the purpose of helping potential contributors to develop maintainable data treatment software of a high quality which can easily be adapted by neutron and muon sources. The guidelines are developed and used in the course of a 4-year Pan-European project to facilitate collaborative development of inter-operable data treatment software among European neutron and muon sources. These guidelines are specific, and over the course of the SINE2020 project helped to harmonize software development within this community. Such specific guidelines should not be uncritically applied to the development of software within another community. Hence, for this reason this paper also presents more generic guidelines for scientific software development from which more specific guidelines applicable to a given community can be created. We hope that these guidelines will help potential contributors to develop data treatment software of a high quality which can easily be adapted by neutron and muon sources. 
...
It has become vital for modern neutron and muon sources to ensure that adequate user software is in existence not only for data acquisition but also for data treatment in order to ensure that the sources produce science rather than just data. The reason for this is at least two-fold 1) the demography of the users is changing from scientists building a career in neutron scattering or muon spectroscopy to scientists that consider neutron scattering and muon spectroscopy as tools among a broader set of tools and are thus not sufficient knowledgeable or skilled about the deeper details behind data treatment for neutron scattering or muon spectroscopy. Moreover, the users may simply not have the skills in how to analyse their data based on programs like MatLab, IDL, Python, or similar. 2) The advances in technology are leading to vast amounts of data that cannot be analysed by hand. For example, some instruments at the
European Spallation Source (ESS) are estimated to have data rates beyond [](TODO: Tobias’ paper) and at modern photon sources the situation is even more severe. This can be compared to the data rates ten years ago, where …(TODO: Historic example). With increasing data rates, the demand for performing ever more complex experiments increases as well as the need for more complex analysis techniques (e.g.\ based on machine learning techniques). The consequence of these aspects is that users inherently will depend on software developed by other developers typically at the facilities or in the user community, which in turn add significant requirements to quality, trustability, reliability and usability compared to when a developer develops software for herself or her colleague sitting next door.  Unfortunately, no neutron or muon source is able to lift the burden of providing adequate data treatment software for data reduction and analysis on their own because the associated user community is widely heterogeneous in terms of skills, instrument techniques and scientific disciplines. There a rather large and extensive software suite is required in order to be comprehensive. Another challenge is that the user community to a large extent depends on software, which on the one hand, has been critical for the development of the domain, but on the other hand suffers from poor documentation, maintainability, technical dept and single-point-of-failure in the sense that it depends critically on a single developer who in many cases started developing the software to solve one of their own problems rather than for the user community, and whose primary profession often is scientist rather than software developer.  Moreover, in many cases software has been developed by a junior scientist (PhD student or post doc) who due to the inherent nature of such positions only can maintain the software for a few years after the first usage of it.
...
\item sharable and inter-operable so that it can be used or adapted by other users or providers than those who developed the software in its first place
\item user-friendly to facilitate the uptake of the software in the user community
\item extensible and facilitating contributions from third party developers to nurse contributions from the user community partly to get additional resources but more importantly to enable incorporation of methodologies developed in academia. 
\end{itemize}

The lack of guidelines can lead to the code being difficult to read, because the style will change depending on who wrote it. In the worst cases this can lead to people believing that the code is incorrect, when it is correct, hence leading to wasted effort. The other potential pitfalls with this isolated approach of software development is that bugs are less likely to be found, due to less users. Also the software may become too cumbersome to develop leading to the project having to start from scratch. Inherently, these factors make it more difficult to share and for other research groups or research infrastructures to adapt the software for their own purpose.
Guidelines for software development ensure that the code is of a high quality and that the project progress effectively. A good set of guidelines will help the software be stable, inter-operable and extensible. Therefore, it will be easier to continue growing the code base to include new scientific analysis and techniques and to adapt the software for instance at other facilities that the one that developed the software in the first place. 

While there is an extensive technical literature on the best practices to develop large software projects (e.g.\ "The Mythical Man-Month", Frederick P. Brooks, Addison-Wesley, 1995; "Ship it! A practical guide to successful software projects", J. R. Richardson and W. Gwaltney, Jr., O'Reilly, 2005; ...), different domains have specific needs and requirements that may lead to different choices tailored to their specific user cases. For scientific software only a few references presenting useful guidelines exist (e.g.\ "Effective Computation in Physics", A. Scopatz and K. D. Huff, O'Reilly, 2015; G. Wilson et al., PLoS Biol 12(1): e1001745, 2014; J. Hastings et al., GigaScience 2014, 3:31, L. B. Silva et al., F1000Research 2017, 6:273; G. Wilson et al., PLoS Comput Biol 13(6): e1005510, 2017; https://software-carpentry.org/). 

The guidelines presented here are based on answers to a questionnaire to developers of data treatment software that are considered in SINE2020, namely (in order of age) McStas \cite{McStas}, SasView \cite{SasView}, Mantid \cite{Mantid}, MuhRec / KipTool \cite{MuhRec}, \cite{KipTool}, and BornAgain \cite{BornAgain}.  The environment and processes surrounding these software packages are quite different spanning from small teams, where guidelines are needed for getting the software adapted by a larger user community and for getting more developers involved to ensure the software’s continuous success, to larger teams of around 20 developers distributed across the world, where guidelines are needed to ensure a consistent quality across the code base. In the context of the SINE2020 EU project, the common good practices identified among those projects have been proposed as guidelines that should be followed by any programmer (scientist of professional software developer) who contributes to data treatment software for the neutron and muon community. 

The guidelines are in short:
\begin{itemize}
      \item Use a license compliant with the Open Source Initiative\cite{osi}, such as GPL\cite{gpl}.
...
Although the guidelines are aimed specifically for the neutron and muon community, the guidelines to a large extent reflects in our opinion what is common practice in scientific software (check references). However, when it is appropriate the idea of the guideline will be abstracted so that an alternative can be chosen in the future. We have made that choice because technology choices tend to be rather short lived. For instance, Python is today the de facto standard scripting language in scientific computing even though the first scientific python conference to the best of our knowledge was held as late as in 2002 with the SciPy conference in Pasadena \cite{millman-vaught08}.

\section{Business as Usual}
\label{Business as Usual}

This section describes the day-to-day work of the developers working on the software.
It will discuss the typical tasks performed, such as development methodologies, code review and coding standards.
...
\emph{TODO}
\begin{itemize}
    \item Merge with Project Management
    \item What do we currently do at facilities
    \item Issues with Stakeholder engagement
    \item as software teams get professionalized challenges with user interaction.
\end{itemize}
...
The two extremes of software development methodologies are called "agile" and "waterfall" (TODO: refs for each).
...
Conscious consideration of coding style is an important aspect of producing code of a high quality. The reason for this is two fold; code that conforms to accepted standards is more likely to be well formatted, readable and contain fewer bugs.
...
If multiple languages are going to be used it is possible to have rules for each language, see Mantid \cite{Mantid} as an example.
 ...
 * @param pair_potentails :: [input] A vector of the pair
 *      potential experienced by each atom.
...
To facilitate collaboration and/or the future adoption and extension of the code by other groups, identifier names, comments and documentation should be written in English.
...
Linters are available as part of standalone packages that produce result reports and which are typically easy to install and run.
...
For a wider selection of the available tools it is worth looking over the selection on Awesome Static Analysis \cite{Awesome-Static-Analysis}, which is a community curated list of static analysis tools.
...
\emph{todo}
\begin{itemize}
    \item Summarize what the SINE2020 projects use or insert and refer to table
\end{itemize}

Tracking the tasks (i.e.\/ issue tracking) for a collaborative software project is essential. This is often implemented in two parts; low level tracking of implementation tasks and high level tracking of project goals. The purpose of issue tracking is to provide a description of the task and, particularly for the low level tasks, to assign it to a developer.

For low level task tracking, simple tools such as GitHub Issues are likely sufficient. 


Additional features may include Scrum or Kanban boards, task dependency and task hierarchy. Most cloud hosted Source control management (SCM) services (see next Section \ref{Source control management}) provide their own issue tracking tool, which typically suit the needs of most software projects.
...
High level tracking, typically at project management level, should monitor the project's medium to long term goals and their progress. Some cloud based issue tracking systems also support the linking of lower level tasks to higher level goals (e.g.\/ Jira through epics and GitLab through milestones). 
These goals should ultimately be defined by users of the software, see \ref{se}.
...
Hence, SCM is essential for collaborative software projects but is also a useful bookkeeping tool for single developer projects. There are several SCMs available, but Git is unanimously used by all software projects considered in SINE2020 and is therefore strongly recommended for the sake of supporting inter-operability.  

To fully utilise the benefits of SCM, it is necessary to follow a common set of rules, or workflows, for its usage. A workflow defines how developers interact with the SCM. 
...
Hence, there are ample guidelines for how to make best use of SCM through workflows, see for instance Ref.~\cite{awesome-git} for workflows associated with git. Therefore, this section will only give an overview and highlight a few commonly overlooked points.
...
A simple and popular option is the feature branch workflow (see for instance Ref.\ \cite{Atlassian-branching} and \cite{chacon-straub14}).
...
For further reading please see Ref.\/ \cite{git-commit, chacon-straub14}. 

Git \cite{Git} is the most common and one of the most powerful SCM tools used in open source software, and, as mentioned, the recommended SCM for data treatment software project in the domain of neutron scattering and muon spectroscopy. 
One advantage is that it has a wide variety of training resources available such as the e-learning site Learn Git Branching \cite{learngitbranching}.
...
For a distributed team it is beneficial to use a cloud Git repository (opposed to local hosting on site) to avoid network related bottlenecks. The projects queried in SINE2020 unanimously use the cloud Git repository provider GitHub,  \ref{tab:summary}, but when hosting project locally, GitLab is the preferred choice.

\begin{table}[t]
\caption{Summary of responses to questionnaire about technologies and methodologies employed by data treatment software projects}
\label{tab:summary}
\begin{tabular}{c|c}
     &  \\
     & 
