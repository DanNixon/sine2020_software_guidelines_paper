...
Seven of the partners contributed to WP10 comprising six European neutron scattering and muon spectroscopy facilities (ESS, FZJ, LLB, ILL, ISIS, PSI) and University of Parma. 
Technical University of Delft and Technical University of Munich, which both operates a research reactor as well as the HZB research reactor and Technical University of Denmark participated as observers in the work package.   

The first deliverable (D10.1) was to write the standards and guidelines for software developed in SINE2020 and the community as such with the aim of fostering development of sustainable, collaborative and interoperable software. 
To this end, the domain of scientific computing was considered in addition to an analysis of five programs considered in SINE2020. 
A brief description of those programs are given below followed by a discussion of specific guidelines for the neutron scattering and muon spectroscopy domain. 

\subsection{Analysed data treatment software for neutron scattering and muon spectroscopy}
The guidelines for data treatment software for the neutron scattering and muon spectroscopy communities are based on a questionnaire that five existing software projects have completed. Here we give a brief description of those software projects:
\begin{description}
\item[BornAgain, www.bornagainproject.org] BornAgain started out as a reimplementation of the software XXX for simulation of a specific scattering technique grazing incidence Small Angle Scattering.
In the course of SINE2020 it has been extended to other reflectometry techniques.
The project started nine years ago and have around three dedicated full time developers. 
...
\item[Mantid, www.mantidproject.org] Mantid is a project initiated by ISIS eleven years ago and since then joined by SNS, ESS, and ILL, with the purpose of providing data reduction software for all their instruments. Besides these facilities Mantid is also being used for specific instruments at other neutron (and muon) sources such as SINQ, FRM-II, and ANSTO. Mantid is developed in C++ and Python with a GUI based on Qt. 
The software is hosted on GitHub under the GPL3 license and the project has about 20 full time developers across all facilities involved.
It is by far the largest software project specific for data treatment at neutron and muon sources and has annual developer and user meetings, a XXX. 

\item[KipTool \& MuhRec, www.neutronimaging.ch] KipTool and MuhRec are analysis software for the neutron imaging technique, where KipTool is used for quantitative analysis and MuhRec for computed tomography and has started as a spare time project for an instrument scientist who spent his time coding when commuting to and from work on the train. 
...
\item[McStas, www.mcstas.org] McStas is per se not a data treatment software but used for performing instrument simulations.
In was included in the analysis due to its widespread usage in the neutron scattering community. 
It is also the longest living among those analyzed with more than 20 years of effort behind it but only has one dedicated full-time developer associated to it. 
McStas is developed primarily in C and has recently been migrated to GitHub.
It is licensed under the GPL2 licence. 

\item[SasView, www.sasview.org] SasView has only had dedicated full time developers when externally funded and has otherwise depended on the good will of scientists putting time into the project.
SasView has had at most two full time developers dedicated to it but has contributions from around fifteen developers from both photon and neutron sources, universities and other research institutions world wide. 
These developers often contribute during code camps held once or twice per year. 
...
Prior to SINE2020, SasView had a GUI based on WX and no Python interface.
In the course of SINE2020, it has been equipped with a Python interface as well as a GUI based on Qt (from version 5). 
...
\begin{sidewaystable}
 ...
 $1000\times \mathrm{Number}$ of lines of code (kLOC) &  & 2450 & 2968 & 777 & 242 \\
