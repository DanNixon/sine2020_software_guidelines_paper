...
\begin{abstract}
Scientific software is an essential tool for the modern researcher, not the least for users of neutron and muon sources and other large-scale user facilities.
At these facilities advanced data treatment software is required in order to get from data to scientific results. 
It is therefore vital that software is in existence that can be relied on now and in the future for the non-expert as well as the expert users of those facilities. 
In this paper we present a set of guidelines for the scientific community that will facilitate the production of reliable and sustainable software in a community that has no strong tradition for doing so. 
The presentation of generic guidelines is followed by a presentation of a set of specific guidelines, including technology choices, for development of data treatment software for the neutron scattering and muon spectroscopy community. 
These guidelines are based on an analysis of existing software in the community and closely resemble what we envision can be considered the de facto standard for development of data science software. 
Following these guidelines will make it easier to integrate the software with other software and for facilities to adapt the software for their specific needs and that of their user community
\end{abstract}
