...
The specific guidelines for the neutron and muon communities are based on an analysis of existing practice for software considered in SINE2020, namely  BornAgain \cite{bornagain}, Mantid \cite{mantid}, ImagingSuite \cite{ImagingSuite}, McStas \cite{mcstas} and SasView \cite{sasview}.
...
For Python based projects Python's {\it setuptools} is typically sufficient, for specific cases (e.g. complex package dependencies) Conda may be used as an alternative.
...
For example, Microsoft Azure DevOps \todo{cite} offers a free service that will compile builds and run tests over a fixed time limit (available for Linux, Mac OS and Windows).
...
For example, Jenkins \cite{Jenkins} is a self hosted CI solution that requires the user to provide the infrastructure.
As a result Jenkins can be adapted to suit the specific requirements of the software project.
...
The software that participated in WP10 of SINE2020 are BornAgain \cite{bornagain}, Mantid \cite{mantid}, McStas \cite{mcstats}, ImagingSuite (MuhRec and KipTool) \cite{ImagingSuite}, and SasView \cite{sasview}.
...
\begin{itemize}
      ...
      \item Support community standards for data formats where such exists, specifically loading of NeXus/HDF5 data formats where relevant.
      \item Use Git for source code management, including user and developer documentation.
...
These guidelines contain specific recommendations for the technologies that should be used (e.g. C++ and Python for programming languages), based on what is currently available.
...
\begin{sidewaystable}
 ...
 Plotting library & Matplotlib & Matplotlib & ?? & QtChart & Matplotlib \\
 \end{tabular}
