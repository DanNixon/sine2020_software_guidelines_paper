...
This section discusses factors and guidelines that affect the day-to-day work of software developers.
Such as development methodologies, code review and coding standards.
...
Two extremes of software development methodologies are called "agile" and "waterfall".
In the following we will discuss these two extremes.
...
SCRUM is a popular agile methodology where a sprint has a pre-determined length, typically of a few weeks.

In contrast to this, each step in the development cycle (i.e., requirements, specifications, design, implementation, testing in that order) are fully completed for the project before moving on to the next task.
This potentially means that a mistake in, for instance, the design first is discovered towards the very end of the project, making it costly to fix. 
The water fall approach, or at least the part considering gathering of requirements and specifications, has for instance been used at neutron and muon sources when the actual implementation is outsourced and a fixed price is desired from the provider.

In reality agile development is often linked to a long term plan with high-level requirements, milestones, etc. 
...
Code should be of a high quality, the reason for this is two fold; code that conforms to accepted standards is more likely to be well formatted, readable and contain fewer bugs \cite{Green_2011}..
...
If multiple languages are going to be used it is possible to have rules for each language, see Mantid as an example \cite{mantidguidelines}.
A community curated list of standards organised by language is available online at \cite{Awesome_Guidelines}.
Although not commonly used in the community represented by the authors, we note that there exist more advanced analysis tools that give hints about the sustainability and maintainability of the software. 
For instance the Black Duck Open Hub \cite{BlackDuck} and Wily for Python programs \cite{AnthonyShaw}.
...
\begin{itemize}
  \item{A Doxygen \cite{doxygen} (TODO: add citation) comment is present.}
...
In both of these cases it is better to first determine if any additional optimisation or generalisation is guaranteered to become genuinely beneficial.
...
Additional features may include task boards specifically for the methodology employed (e.g.Scrum or Kanban boards), task dependency and task hierarchy.

Most cloud hosted Source control management (SCM) services (e.g., GitHub and GitLab) provide their own issue tracking tool, which typically suit the needs of most software projects.
...
Hence, there are ample guidelines for SCM, for more information please see  Ref.\ \cite{awesome-git} for a list of existing ones.
...
A simple and popular option is the feature branch workflow ( Ref. \cite{feature-branch})).
In this workflow a developer will work on a new feature using a new branch (also called fork) and that branch is then merged when the changes are accepted.
There is some empirical evidence that  software projects supporting branching generally are sustained for longer compared to projects where branching is not used \cite{Alhozaimy2017}.
...
The software considered in Section \ref{SINE2020} are all hosted on GitHub \cite{github}. 
Some of the other software considered in SINE2020, but not part of the analysis for guidelines, are hosted on GitLab \cite{gitlab}. 
...
For example, it has no restrictions for build times, stores data locally and gives the developer complete control over the setup of the build environment.
 Jenkins is used unanimously by the software projects presented in Section \ref{SINE2020}.
...
This should also include if the commit history is sensible and the relevant documentation (including release notes and user documentation) has been updated.
