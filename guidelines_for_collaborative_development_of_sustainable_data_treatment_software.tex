...
Python is becoming the standard language for data science and as such has a wide range of libraries, documentation and resources available for it \cite{python-nature}, with some of the most commonly used organized under the SciPy.org umbrella \cite{scipy}.
Python also works fluently with the Jupyter electronic notebook, an increasingly popular tool in the scientific community \cite{jupyter-nature} and frequently used for tutorials.

If the application requires a graphical user interface (GUI) then a good choice of library is Qt \cite{Qt}.
Qt is an application framework commonly used by many open source and commercial software packages and is also a popular framework for GUI development in the neutron scattering and muon spectroscopy community (c.f., Section \ref{SINE2020}.
