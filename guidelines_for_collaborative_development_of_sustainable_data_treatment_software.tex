...
The report \cite{sine2020_wp10_d10_report} contains the standards and guidelines that were developed and used for WP10 of SINE2020. The guidelines are based on an analysis of the programs BornAgain, Mantid, McStas, MuhRec / KipTool, and SasView, and 
the report contains questionnaires, statistics and information about these programs including tables identifying commonality in the beginning of the SINE2020 project.
...
We consider the guidelines to be both realistic and pragmatic because they are based on existing practise in the aforementioned five software projects, which are rather diverse, but also have enough commonalities to form the basis for the guidelines. 
We note that many newer software projects for data treatment software and, in more general terms for data science software, in practise follows these guidelines to a large extent. 
For instance cctbx and F.O.X. for crystallography and scikit-learn or matplotlib for machine learning and plotting, respectively. 

Unlike the generic guidelines, these guidelines contain recommendations for technology choices (e.g., for programming language) and they therefore also reflect the time in which they have been developed. 
As technology evolves these guidelines will have to be updated and they should therefore be reviewed on a regular basis. 
Use of the same technology are particularly important for facilitating inter-operability, collaboration, and sharing of code, because it obviously makes it easier for other to adapt software if they already are familiar with the technologies employed. 

The diversity and commonalities of the analyzed software projects can be realised from Table \ref{summaryTable} that lists a number of key parameters for the projects as of December 2019. 
...
\begin{sidewaystable}
 ...
 \end{tabular}
 \caption{Key parameters for the different software projects analyzed in SINE2020 at the end. 
...
The question about which plotting module to use was not addressed in SINE2020.
We have included it here because this is a choice most software projects in the domain will have to make and it can have ample consequences for the future use and inter-operability of the software. 
As can be seen from Table \ref{summaryTable} MatPlotLib is a common choice and it also has a large developer community supporting it, facilitating its long term sustainability. MatPlotLib has the advantage that it works seamlessly with Qt and also can be used seamlessly from a Jupyter notebook or a Python prompt in a terminal and it therefore seems to be a sensible choice for software with both a Python scripting interface and a GUI. 
