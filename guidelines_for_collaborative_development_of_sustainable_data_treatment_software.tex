...
\subsection{Build Systems (BS)}
\label{Build systems}

The code base of your software should be managed using an appropriate Build System (BS), which will depend on the choice of programming language(s).
The BS handles the dependency finding and build file generation native to the host platform.
...
Automated testing infrastructure is thus essential in order to provide reliable software product.
There is a plethora of testing frameworks available nowadays; almost every modern programming language ships a unit testing framework with its standard libraries, e.g. {\sc CxxTest} for {\sc C++}, {\sc unittest} for {\sc Python}, {\sc JUnit} for {\sc Java}, etc. 
Moreover, there are third party, more advanced testing frameworks, such as {\sc GoogleTest}, {\sc GoogleMock}, that provide extended techniques for testing, for example, mocking.
...
This guarantees that the same misbehaviour will not reoccur in the future.
...
For example, an algorithm that is supposed to find a peak may operate as expected if there is a well defined peak in the data, but fail if the statistics for a certain measurement was low.
Therefore, having unit and integration tests with exactly calculated expected outputs is often not practically feasible.
In such cases reproducibility tests can be used. 
Those do not, strictly speaking, test the output of the code to be correct, but they ensure that the output remains reproducible over time.
...
Those are tests that execute a certain aspect of the software over sufficiently large datasets.
They do not necessarily check the output, but they record the execution time.
Monitoring the evolution of the performance tests timings, one could automatically detect when a newly suggested code change increases significantly the execution time of certain operations.

When it comes to automated testing, the GUI is arguably the layer that is the hardest to test.
From the other hand, most often bugs are likely to be found in the graphical interfaces.
However, there are techniques and tools that allow sufficient automation of GUI testing.
Among those is the separation of the logic into tiers, e.g. Model-View-Presenter (MVP), where the model and the presenter can be covered by unit tests with reciprocal mocking.
Besides, there are tools available, that allow the test automation of the views themselves.

While automated testing is indispensable for any software project, the manual (or unscripted) testing should not be undermined.
Those should be done periodically by the developers or a dedicated QA team, typically before the releases or other milestones.
...
\subsection{Deployment and Computing Infrastructure}
